...
# A cheat-sheet: http://www.gardenersown.co.uk/Education/Lectures/R/   #
...
acs = read.csv(text=readLines("./ACS_16_5YR_B25077_with_ann.csv")[(-2)])
...
# Join other data to 'dof' data using "match" (will work regardless of whether data are sorted)
...
# In addition to the functions included with the base version of R, there are THOUSANDS of 
...
# Install packages using a single line
...
plot(counties["ALAND"], breaks="jenks", key.pos=2, pal=sf.colors(10), axes=TRUE)
...
# A handy trick is to be able to summarize data.  Let's summarize the dof data by the categorical column 'mpo' and put the data you want summarized before it, separated by plus signs
...
barplot(dofsum$pop17.sum, names.arg=dofmean$mpo)
